...
**City Pulse synthesis** (`pulse.py`) — hourly situational summary that synthesizes all active alerts into actionable intelligence. Runs as a standalone cron job in the poller container. Uses editable prompt templates from `prompts/` (YAML frontmatter + template body), seeded to the data volume on first start. Thinking is enabled (`thinkingBudget: 4096`) for spatial reasoning and cross-source correlation. A daily summary job at 23:00 compresses 24 hourly pulses into a one-paragraph digest, which is fed as multi-day trend context into future hourly pulses. Health is tracked via `pulse_ok()`.
