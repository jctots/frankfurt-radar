...
**City Pulse synthesis** (`pulse.py` + `pulse_categories.py`) — hourly situational summary combining deterministic analysis with LLM synthesis. Only ongoing alerts (active disruption window) count toward trends; future and expired alerts are excluded. Each alert is severity-weighted based on source-specific fields (DWD severity level, RMV service type, closure vs. warning, event impact). Category statuses and trends are computed by script using an adaptive 7-day EWMA baseline on these weighted scores; the LLM writes the summary text and recommendation using pre-computed categories with explicit field guidance (status, trend, count, ewma). `travel_ok` is computed deterministically. Runs as a standalone cron job in the poller container. Uses editable prompt templates from `prompts/` (YAML frontmatter + template body), seeded to the data volume on first start. Thinking is enabled (`thinkingBudget: 4096`) for spatial reasoning and cross-source correlation. A daily summary job at 23:00 compresses 24 hourly pulses into a one-paragraph digest, which is fed as multi-day narrative context into future hourly pulses. Health is tracked via `pulse_ok()`. See [analysis.md](analysis.md) for the full methodology.
